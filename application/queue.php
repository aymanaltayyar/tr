...
if ($curr_time - @filemtime(APP_BASE_PATH."/queue.txt") >= 1) {
    ...
    class cron extends Att
    {
        ...
        function showArts(){
            for ($kid = 1; $kid <=4; $kid++) {
                if ($kid == 1) {$this->Artefact($uid, 1, 3, 'تحفة المهندس المعماري النادرة', $kid);}
                if ($kid == 2) {$this->Artefact($uid, 2, 3, 'تحفة القوات السريعة النادرة', $kid);}
                if ($kid == 3) {$this->Artefact($uid, 3, 3, 'تحفة المستكشف النادرة', $kid);}
                if ($kid == 4) {$this->Artefact($uid, 5, 3, 'تحفة بناء القوات النادرة', $kid);}
                $this->Artefact($uid, 1, 1, 'تحفة المهندس المعماري الصغيرة', $kid);
                $this->Artefact($uid, 1, 2, 'تحفة المهندس المعماري الكبيرية', $kid);
        
                /**
                 * MILITARY HASTE
                 */
                 $this->Artefact($uid, 2, 1, 'تحفة القوات السريعة الصغيرة', $kid);
                $this->Artefact($uid, 2, 2, 'تحفة القوات السريعة الكبيرة', $kid);
        
                /**
                 * HAWK'S EYESIGHT
                 */
                $this->Artefact($uid, 3, 1, 'تحفة المستكشف الصغيرة', $kid);
                $this->Artefact($uid, 3, 2, 'تحفة المستكشف الكبيرة', $kid);
                $this->Artefact($uid, 5, 1, 'تحفة بناء القوات الصغيرة', $kid);
                $this->Artefact($uid, 5, 2, 'تحفة بناء القوات الكبيرة', $kid);

                /**
                 * STORAGE MASTER PLAN
                 */
                 $this->Artefact($uid, 6, 1, 'تحفة المخزن الكبير الصغيرة', $kid);
                $this->Artefact($uid, 6, 2, 'تحفة المخزن الكبير الكبيرة', $kid);
                ...
                switch ($f['type']) {
                    ...
                    case 2: //возврат
                        if($this->returnCom($f['jobID'])){
                            ...
                            $database->query("INSERT INTO critical_log (`id`,`work`,`work_id`,`job_id`) VALUES ('0','возврат','" . $f['id']."','".$f['jobID']."')");
                        }
                        break;
                    case 3: //подкреп
                    ...
                    case 7: //   рынок отправка
                        ...
                        break;
                    case 8: //   рынок возврат и х2/х3 транспорт
                    ...
                    case 10: //   удаление аккаунта
                        ...
                        break;
                    case 11: //   оживление герыча
                        if($this->reviveHero($f['jobID'], $f['if1'])){
                        $database->query("DELETE FROM `queue` WHERE `id`='" . $f['id']."'");
                    }else{
                            $database->query("INSERT INTO critical_log (`id`,`work`,`work_id`,`job_id`) VALUES ('0','оживление героя','" . $f['id']."','".$f['jobID']."')");
                        }
                        break;
                    case 13:
