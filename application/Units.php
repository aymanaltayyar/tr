...
class Units
{
    ...
    private function loadUnits($post)
    {
        ...
        if ((time() < $info['ptime'] || $info['protect'] == 1) && $session->uid!=$owner) {
            $form->addError("error", "الاعب مايزال تحت حماية المبتدئين.");
        }
		...
		if($database->isVillageOases($id) && $post['c'] != 4){
			$form->addError("error", "Nah!");
		}
        ...
        if($data['sent']==0){
            ...
            for ($i = 1; $i <= 11; $i++) {
                ...
                if ($data['u' . $i] > $unitarra['u' . $i]) {
                    $form->addError("error", "لا يمكن إرسال وحدات أكثر مما تمتلك");
                    break;
                }
                if ($data['u' . $i] < 0) {
                    $form->addError("error", "لا يمكن ارسال وحدات بالسالب.");
                    break;
                }
                $sendnum += $data['u' . $i];
            }
            if (!$sendnum) {
                $form->addError("error", "يجب تحديد وحدة على الأقل.");
            }
        ...
        global $form, $database, $village, $session;
            ...
            if (!$database->getVillageState($post['s'])) {
                if ($rallypoint['f39'] > 0) {
                    ...
                    $form->addError("error", "Постройте Пункт Сбора");
