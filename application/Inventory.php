...
if($_POST && $_POST['a']=='inventory'){
	...
	elseif($itemData['btype']==6){
        ...
        $database->editProcItem($itemData['id'], 1, $uid); //одеваем шмотку
        $database->setHeroInventory($uid, 'horse', $itemData['id']); //чет делаем в инвентаре
			$database->modifyHeroFace($uid, 'horse', $itemData['type']);
			$database->modifyHero2('speed', $value, $uid, 1);
	}

	elseif($itemData['btype']==7 && $data['amount']>0){ //походу воскрешает немного сдохших войск 25%  $itemData['type'] тут число уже надетых повязок/хуязок
	if($data['amount'] <= $itemData['num']-$itemData['type']){ //если число введеных повязок меньше или равно их реальном кол-ву
	...
	elseif($itemData['btype']==8 && $data['amount']>0){ //походу воскрешает немного сдохших войск 33%
...
/*
	elseif($itemData['btype']==10 && $data['amount']>0){ //опыт геру свитками!
		if($data['amount'] <= $itemData['num']){
	...
	elseif($itemData['btype']==12){//воскрешаем героя
        $hero=0;
	...
	elseif($itemData['btype']==13){//скидываем очки героя
