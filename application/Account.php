...
require_once("Lang/" . ($language ? $language : 'ar') . "/Lang.php");
...
class Account {
	...
	private function Signup() {
		global $database,$form,$mailer,$generator,$regme,$iRed,$config;
		if(in_array($_POST['name'], array('احا','كسمك','زق','خرا','_l_','_|_','خول'))){
			$form->addError("name",'الإسم غير لائق.');
		}else{
			$array = array(',','!','&','@','#','%','^','*');
			//if( strpos($_POST['name'], ',') !== false ) {
			if ($this->strposa($_POST['name'], $array, 1)) {
				$form->addError("name",'الإسم يحتوي على علامات ممنوعة');
			}
			if(!isset($_POST['name']) || trim($_POST['name']) == "") {
				...
				if(strlen($_POST['name']) < USRNM_MIN_LENGTH) {
					...
					$form->addError("name","Недопустимый Ник");
				}
        ...
        if(!isset($_POST['kid'])) {
            $form->addError("kid","Выберите регион!");
        }
		if(!isset($_POST['agb'])) {
			$form->addError("agree",AGREE_ERROR);
		}
        //if($database->CheckUniqueIP($_SERVER['REMOTE_ADDR'])){$form->addError("agree","С вашего IP Уже был зарегистрирован аккаунт!");}
		if($form->returnErrors() > 0) {
		 ...
		 if(!empty($invbyname)){
            ...
            $uid = $regme->activate($_POST['name'],md5($_POST['pw'].mb_convert_case($_POST['name'],MB_CASE_LOWER,"UTF-8")),$_POST['email'],0,0,$act,$act2,$_POST['invited']);
				if($uid) {
					$iRed->sendActivate($_POST['email'],$_POST['name'],$_POST['pw'],$act);
					if($config['needActivate']){
						//$_SESSION['isOkay'] = 'تم التسجيل بنجاح، يجب تفعيل الحساب من البريد قبل بدأ اللعب.';
						header("Location: activate.php");
					}else{
						$_POST['id'] = $act;
						$this->Activate2();
						//header("Location: activate.php?code=$act");
					}
				}
	 ...
	 function Activate() {
        ...
        $uid=0;
			if(count($dbarray)) {
				...
				$uid = $regme->register($dbarray[0]['username'],$dbarray[0]['password'],$dbarray[0]['email'],$dbarray[0]['tribe'],$dbarray[0]['ref']);
                }
				...
				if($uid) {
                    if($dbarray[0]['ref']>0){
                        ...
                        //пригласившему зачитываем ачивку
                    }
					...
					//$database->query('UPDATE users SET plus = '.(time() + (24*60*60)).' WHERE id = '.$uid.'');
					//$database->query('UPDATE fdata SET f39t = 16, f39 = 1 WHERE vref = '.$wid.'');
					if(OPENING > time()){
	...
	private function Login($isActive = FALSE) {
		...
		if($aData['id']){
			if($config['needActivate']){
				$form->addError("user",'يجب تفعيل الحساب أولا.');
			}else{
    ...
    private function forgotPassword($email) {
        global $database,$generator,$form,$mailer,$iRed;
        ...
        elseif($database->checkProcExist($getData['id'])){
            if($database->checkExist($email,1)){
                ...
                $iRed->sendPassword($email, $getData['username'], $npw, $act);
            }else{
                $form->addError("pw_email",'Email not found');
                $er=true;
            }
        }else{
            $form->addError("pw_email",'وصلتك رسالة بالفعل');
            $er=true;
        }
        if($er) {
            $_SESSION['errorarray'] = $form->getErrors();
