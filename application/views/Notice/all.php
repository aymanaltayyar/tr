...
<?php
    ...
    if ($type == 21){ // Hero
        if($dataarray[1]){
            if($dataarray[1] == 'dead'){
                echo '<div class="heroStatusMessage" style="padding:0 !important;"><img alt="بطلك ميت" title="مات البطل" src="img/x.gif" class="heroStatus101"></div>';
            }else{
                $btype = $dataarray[1];
                $type = $dataarray[2];
                if($btype < 16){
                ...
                $outputList .= "<img title=\"(".$dataarray[3]."x)\" src=\"img/x.gif\" class=\"unit u".(($session->tribe-1)*10+$type)."\"\">";
                    ...
                    echo '<img src="img/x.gif" class="silver" title="'.$dataarray[3].' فضة">';
                }
            }
        }
