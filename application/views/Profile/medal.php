...
if($displayarray['ptime'] > time() && $displayarray['protect']!=0){
    ...
    $profiel = preg_replace("/\[#0]/is",'<img src="'.$gpack.'/t/tn.gif" border="0" title="تبقي لديك '.$uurover.' ساعة من حماية المبتدئين." >', $profiel, 1);
} else {
    $geregistreerd=date('Y/m/d', ($displayarray['ptime']));
    $tregistreerd=date('H:i', ($displayarray['ptime']));
    $profiel = preg_replace("/\[#0]/is",'<img src="'.$gpack.'/t/tnd.gif" border="0" title="سجل الاعب في '.$geregistreerd.' '.$tregistreerd.'">', $profiel, 1);
}
