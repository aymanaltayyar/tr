...
<div id="sidebarAfterContent" class="sidebar afterContent">
	...
	<script type="text/javascript">
		...
		if($market_=="" && $session->plus){
            ...
            echo "التجار: ".($lvlm-$database->totalMerchantUsed($village->wid))."/".($lvlm);
			} else{ 
...
<?php if($session->goldclub != 0){  ?>
                ...
                <button type="button" id="buttonBuild" class="layoutButton build<?=$_COOKIE['builder']?>  green"
                        title="<?php echo $_COOKIE['builder'] == "On" ? 'تعطيل الترقية السريعة' : 'تشغيل الترقية السريعة'; ?>" onclick="return false;">
                    <div class="button-container addHoverClick ">
                        <i></i>
                    </div>
                </button>            
	...
	<div class="sidebarBoxInnerBox">
            ...
            <div class="innerBox content">
				لوحة التحكم الخاصة بالإدارة فقط، لا يمكن الدخول للوحة إلا عبر العضويات ذات صلاحية مدير فقط.<br><br>
                    <div class="questAchievementContainer">
						<a href="panel">
                        <button  class="green" id="button545dec96b3573">
                            <div class="button-container addHoverClick">
                                ...
                                <div class="button-content">الدخول للوحة</div>
                            </div>
						</button>
	</a>
                    </div>
...
<?php } ?>
    ...
    <?php
    $vote = $database->query("SELECT dgold FROM `users` WHERE `id`='".$session->uid."'"); //проверка голосовал ли уже игрок
