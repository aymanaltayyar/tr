...
<div class="boxes-bc" ></div>
	...
	<tbody>
		...
		<td class="element attribName"><?php echo HEROI25; ?></td>
                    ...
                    <tr class="attribute experience tooltip" title="<?php if($session->heroD['level']!=100){  ?><?=HEROI33?>||<?php echo HEROI31.' '; echo($hero_levels[$session->heroD['level'] + 1] - $session->heroD['experience']); ?> <?=HEROI32?> <?php echo($session->heroD['level'] + 1); ?> <?php  }else{ echo 'Достигнут максимальный уровень'; } ?>.">
					...
					<span class="speed tooltip">
						السرعة:<strong id="heroSpeedValueNumber"><?php echo $session->heroD['speed'] * INCREASE_SPEED; ?></strong> <?php echo HEROI38;?></span>
                        </td>
    ...
    ?>
        ...
        <div class="attribute regenerate tooltip"
             ...
             <div class="element attributesHeadline">
                            موطن البطل <a href="karte.php?d=<?php echo $session->heroD['wref']; ?>"><?php echo $database->getVillage($session->heroD['wref'])['name']; ?></a>. سيتم تجديدة في <a href="karte.php?d=<?php echo $village->wid; ?>"><?php echo $village->vname; ?></a>.
                            </div>
                        <div class="element attributesHeadline">لتغيير موطن البطل او تجديدة في قرية أخرى</div>
                        <div class="element attributesHeadline"></div>
                        <div class="roundedCornersBox white">
                            <div class="element resourceDemandCaption">تكلفة التجديد:
                            </div>      
        ...
        <div class="inlineIcon duration"><i class="clock_medium"></i><span class="value "><?php echo $generator->getTimeFormat(($hero_t[$session->heroD['level']]['time'] / SPEED * 1.5)); ?></span></div>             
                ...
                <?php }else{ // hero is in revive
                    ...
                    <div class="heroStatusMessage header warning">
                        ...
                        يتم تجديد البطل في <a href="karte.php?d=<?php echo $session->heroD['wref']; ?>"><?php echo $database->getVillage($session->heroD['wref'])['name']; ?></a> : الوقت المتبقي <span class="timer " counting="down" value="<?php echo ($session->heroD['revivetime'] - time()); ?>"><?php echo $generator->getTimeFormat($session->heroD['revivetime'] - time()); ?></span>
...
<h4>
    <div class="openCloseSwitchBar">
        <img alt="السمات" src="img/x.gif" class="openedClosedSwitch switchOpened">
        <span class="title">السمات</span>
        <span class="heroAttributesFormMessage notice hide">يرجى حفظ التغييرات</span>
        <div class="clear"></div>
    </div>
</h4>
...
<div class="clear"></div>
    ...
    <table id="attributesOfHero" cellspacing="0" cellpadding="0" class="transparent attributes">
        ...
        <tr id="attributepower" class="attribute power" title="<?=HEROI8?>||<?=HEROI7?><br><span class='heroAttributeInformation'>الكفائة الحربية: ‎‭‭<?php echo (100 + $tp * $session->heroD['power'] + $session->heroD['itempower']); ?> من البطل</span>">
