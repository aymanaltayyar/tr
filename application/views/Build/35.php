<h4 class="round spacer">قائمة الإحتفالات</h4>
	...
	<tbody>
		...
		$i = 1;
			...
			if($session->gold >= 3 && $building->getTypeLevel(17) >= 1) {
				...
				<div class="button-content">تبادل الموارد</div></div></button>
				...
				if($inuse > $time){
					echo "<td class=\"act\">
					<div class=\"none\">هناك إحتفال قائم</div>
					</td></tr>";
					}
					...
					if($village->getProd("crop")>0){
		                ...
		                echo "<br><span class=\"none\">Ресурсов будет достаточно ".$time[0]." в ".$time[1]."</span></div></td>";
					} else {
						echo "<br><span class=\"none\">Не хватает зерна.</span></div></td>";
					}
                    echo "<td class=\"act\"><div class=\"none\">Не хватает<br>ресурсов</div></td></tr>";
                }
                else {
					echo "</td>";
                    echo "<td class=\"act\">";

					echo "<a class=\"research\" href=\"dorf2.php?pivo&wid=$village->wid\">إحتفال</a></td></tr>";
                }
...
<?php
		...
		if($timeleft > Time()){
			?>
				<h4 class="round spacer">إحتفالات جارية</h4>
			<?php
            ...
            echo "</span></td>";
            echo "<td>الإنتهاء في ".date('H:i', $timeleft)."</td></tr>";
			echo "</table>";
            $timer +=1;
		}
?>