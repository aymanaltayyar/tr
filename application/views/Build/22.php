...
<?php
	if ($building->getTypeLevel(22) > 0) {
		...
		?>
		<div class="clear"></div>
		<h4 class="round">بحث القرى <?php echo $village->vname; ?></h4>
		<?php
		$fail = $success = 0;
			...
			if($technology->meetRRequirement($i) && !$technology->getTech(($startTech)) && !$technology->isResearch(($startTech),1)) {
				...
				if(${'r'.$i}['wood'] > $village->maxstore || ${'r'.$i}['clay'] > $village->maxstore || ${'r'.$i}['iron'] > $village->maxstore) {
					echo "<br><div class=\"contractLink\"><span class=\"none\">
							يجب تطوير مخزن الخام</span></div></div></div></div>
							<div class=\"clear\">&nbsp;</div>";
				}
				else if(${'r'.$i}['crop'] > $village->maxcrop) {
					echo "<br><div class=\"contractLink\"><span class=\"none\">
							يجب تطوير مخزن الحبوب</span></div></div></div></div>
							<div class=\"clear\">&nbsp;</div>";
					...
					if($village->getProd("crop")>0){
						...
						<div class=\"contractLink\">
							<span class=\"none\">ستتوفر الموارد الازمة ".$time[0]." في ".$time[1]."</span>
						</div></div>
		<div class=\"clear\">&nbsp;</div>
		</div>";
					} else {
		...
		/*if($ii == 0){
			...
			<div class="noResearchPossible">
				<span class="errorMessage">تم البحث عن جميع القوات بالأكاديمية.</span>
			</div>
		<?php
		...
		if(count($acares) > 0) {
			...
			foreach($acares as $aca) {
				...
				echo "<td class=\"fin\"><span>".$date[1]."</span><span> ساعة</span></td>";
				echo "</tr>";
				$timer +=1;
			}
			echo "</tbody></table>";
