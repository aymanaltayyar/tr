        ...
        global $form, $database, $session;
            if (isset($post['a_name'])) {
                ...
                if ($this->userPermArray['opt4'] == 0) {
                    ...
                    // iRedux
                    $database->sendMessage($UserData['id'], 0, 'وصلتك دعوة تحالف','وصلتك دعوة من التحالف <a href="allianz.php?aid='.$aid.'">'.$aData['name'].'</a> قم بالدخول للسفارة لقبول أو رفض الدعوة.', 0, 0, 0, 0,0);
            ...
            foreach ($this->inviteArray as $invite) {
                if ($session->alliance == 0) {
                    if ($invite['id'] == $get['d'] && $invite['uid'] == $session->uid) {
                        ...
                        // عدد أعضاء التحالف الماكس $alliance_info['max']
