...
$this->addTech($vref);
	...
	*****************************************/
    ...
    function getUserSes($name) {
        ...
        $p=array('N'=>$name);
        return $this->row("SELECT id,password,email,access,plus,brewery,tribe,silver,lastupdate,alliance,sit1,sit2,regtime,protect,cp,gold,ok,deleting,b1,b2,b3,b4,goldclub,invited,evasion,quest,deleting,advtime,lang,apall,dpall,msg,vid,stime,ptime FROM users where `username` =:N",$p);
...
$artvalue[$i]=(!isset($allsamearts[$i])  or count($allsamearts[$i])==0)?$artefactinfo[$i]['default']:((count($allsamearts[$i])==1)?($allsamearts[$i][0]):(max($allsamearts[$i])));
 ...
 return $artvalue;
    ...
    function trainingComplete($vref) {
        ...
        $trainlist = $this->getTrainingList($vref); // берем список тренировок для опр.деры

        if(count($trainlist) > 0){ //если есть че строить..

            foreach($trainlist as $train){ //что б можно было работать с несколькими массивами если строится более 1 пачки

                if(!MOMENT_TRAIN){

                $timepast = $train['lastupdate'] - $time; //а это типо прошла хотя б секунда с прошлой постройки( т.е. -1..-2 и т.д. будет результ)

                $timepast2 = $time - $train['lastupdate']; //ну эта хуйня короче считает сколько сек прошло с момента последней постройки юнитов



                if($timepast <= 0 && $train['amt'] > 0 && $timepast2 >= $train['eachtime']) { //условия короч

                    $trained = 0; //тренировано в итоге

                    $avaibleunits=floor($timepast2/$train['eachtime']); //сколько юнитов помещается в заднице у пэрис хилтон,шучу,не у пэрис хилтон

                    $trained += $avaibleunits; //добавляем тренированых

                    if($trained >= $train['amt']){ //если тренированых больше чем тренируется-хватит это терпеть,затычка в действии

                        $trained = $train['amt'];} //натренировано-тренированым

                    $unit=$train['unit']; //юнит равен типу юнита типа
                    ...
                    if($unit>10 && $unit<=20){$unit=$unit-10;}//если это галлы то обрезаем на 10см(а рим не согласен на обрезания *печальный еврей*

                    }else{$unit=99;}

                    $this->modifyUnit($train['vref'],array($unit),array($trained),1); //добавляем юнитов в деру
        ...
        return $this->query($q,$p);
			...
			function modifyResource($vid, $wood, $clay, $iron, $crop, $mode) {
				...
				$q = "UPDATE vdata set `wood` = '".round($dwood)."', `clay` = '".round($dclay)."', `iron` = '".round($diron)."', `crop` = '".round($dcrop)."' where `wref` = :V";
				 ...
				 $this->query($q,$p); return true;}else{return false;}
...
//???????? ???????? ?? ????????????????/??????????????
    ...
    //ВСЕ ФУНКЦИИ СВЯЗАННЫЕ С ГЕРОЕМ
...
//герой
    ...
    function addHero($uid){
        ...
        //нет смысла защищать и хэш в пизду
...
return $somassive;
...

    {
        ...
        $q = "SELECT u.access,w.x,w.y,w.occupied,u.regtime,u.protect FROM (((
        wdata as w
...
elseif ( strpos($agent, 'Trident/7') ) $body_browser_class = 'ie11';
    ...
    function getUserMSG($username){
        ...
        if($uData['id']){
            ...
            $m = '<span style="font-style:italic;">لم يتم العثور على اللاعب</span>';
        }
