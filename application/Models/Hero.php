        ...
        switch($this->getHeroStatus()){
            case 9:
                ...
                if($mData['sort_type']){ // 
                    ...
                    البطل في المغامرة <a class="" href="karte.php?x='.$advCoors['x'].'&amp;y='.$advCoors['y'].'">&lrm;&#8237;<span class="coordinates coordinatesWrapper"><span class="coordinateX">('.$advCoors['x'].'&#8236;&#8236;</span><span class="coordinatePipe">|</span><span class="coordinateY">&#8237;'.$advCoors['y'].'&#8236;&#8236;)</span></span>&#8236;&lrm;</a>. الوقت المتبقي <span class="timer" counting="down" value="'.$reqTime.'">'.$generator->getTimeFormat($reqTime).'</span> في '.date('H:s', $mData['endtime']).'.<div class="subMessage"> يمكنك رؤية مجريات المغامرة <a href="build.php?gid=16&amp;tt=1&amp;newdid=26019">نقطة التجمع</a>.) </div>';    
                }else{
                    return '<img alt="Held tot!" src="img/x.gif" class="heroStatus9">
				    البطل في طريق العودة.';
            ...
            case 103:
				...
				البطل يدافع في القرية <a href="karte.php?d='.$enf['vref'].'">'.$vData['name'].'</a>.';
            break;

            case 100:
                $vData = $database->getUserVillInfoByVillageID($this->heroData('wref'));
                return '<img alt="Held tot!" src="img/x.gif" class="heroStatus100">
				البطل حالياً في القرية <a href="karte.php?d='.$this->heroData('wref').'">'.$vData['name'].'</a>.';
            break;
        }
