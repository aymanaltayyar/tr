        ...
        for($x=1;$x<=4;$x++){
            ...
            foreach($this->getP($x) as $row){
                ...
                if($row[$b] != 0){
                    ...
                    //$database->modifyGold($row['id'],MEDAL_GOLD,1);
                    //$Msg = 'لقد ربحت '.MEDAL_GOLD.' ذهبة نتيجة حصولك على الرتبة '.$i.' في مدافعون الأسبوع.';
                    //$database->sendMessage($row['id'], 6, 'ربحت ذهب',htmlspecialchars(addslashes($Msg)), 0, 0, 0, 0,0);
                }
            }
        }
