...
Class Plus{
    ...
    function upAll(){
        ...
        if($rGold <= $session->gold){
            if($this->checkupAll()){
                ...
                return $this->errorMsg('جميع الحقول مستوي 20 بالفعل');
            }
        }
    ...
    function upStorage(){
        ...
        if(25 <= $this->session->gold){
            ...
            return $this->errorMsg('لا تمتلك ذهب كافي');
        }
    }
    ...
    function smithyUpgradeAllToMax(){
        ...
        if($this->smithyAll()){
            ...
            return $this->errorMsg('تم تطوير جميع الوحدات التي تم البحث عنها بالفعل بهذه القرية');
        }
    }
    ...
    function academyResearchAll(){
        ...
        if($this->academyAll()){
            ...
            return $this->errorMsg('تم البحث عن جميع وحدات هذه القرية');
        }
    }
    ...
    function moreProtection($type){
        ...
        if($gCost <= $this->session->gold){
            //if($this->pAData('addonprotection') < 6){
                //if($this->pAData('addonprotection') - $bonus < 6){
                    ...
                    return $this->reloadPage();        
                /*}else{
                    return $this->errorMsg('يمكنك فقط شراء '.(6 - $this->pAData('addonprotection')).' ساعة.');
                }*/
            /*}else{
                return $this->errorMsg('أقصي حد 6 ساعات يوميا.');
            }*/
        }else{
            return $this->errorMsg('لا تمتلك ذهب كافي');
        }
    ...
    function updatePlus($type){
        global $database,$session;
        switch($type){
            case 1: // Plus
                $session->plust 
                ? $session->plust > time() 
                ? $nTime = $session->plust + PLUS_PRODUCTION 
                : $nTime = time() + PLUS_PRODUCTION 
                : $nTime = time() + PLUS_PRODUCTION;
            ...
            case 2: // Wood Prod
                $session->bonus1 
                ? $session->bonus1 > time() 
                ? $nTime = $session->bonus1 + PLUS_PRODUCTION 
                : $nTime = time() + PLUS_PRODUCTION 
                : $nTime = time() + PLUS_PRODUCTION;
            ...
            case 3: // Clay Prod
                $session->bonus2 
                ? $session->bonus2 > time() 
                ? $nTime = $session->bonus2 + PLUS_PRODUCTION 
                : $nTime = time() + PLUS_PRODUCTION 
                : $nTime = time() + PLUS_PRODUCTION;
            ...
            case 4: // Iron Prod
                $session->bonus3 
                ? $session->bonus3 > time() 
                ? $nTime = $session->bonus3 + PLUS_PRODUCTION 
                : $nTime = time() + PLUS_PRODUCTION 
                : $nTime = time() + PLUS_PRODUCTION;
            ...
            case 5: // Crop Prod
                ...
                ? $nTime = $session->bonus4 + PLUS_PRODUCTION 
                : $nTime = time() + PLUS_PRODUCTION 
                : $nTime = time() + PLUS_PRODUCTION;
