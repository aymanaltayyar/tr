        ...
        if($_POST['toggleAutoprolong']){
            ...
            if(!$vil['natar'])
            {
                if($gold >= 3)
                {
                    ...
                    }elseif($_POST['m1'][0]<=$vil['wood'] && $_POST['m1'][1]<=$vil['clay'] && $_POST['m1'][2]<=$vil['iron'] && $_POST['m1'][3]<$vil['crop']){ //если зерно во время расчета ушло в минус
                        if ($vil['crop'] > 0) {
                            $crop = $_POST['m1'][3] - $vil['crop']; //разница в кропах которую будет отнимать от распределенного зерна чтобы компенсировать минус
                        } else {
                            if($_POST['m1'][3]>0){
...
*/
        ...
        $html = '<div id="build" class="exchangeResources"><p class="npc_desc">مع تاجر المبادلة يمكنك توزيع الموارد كيفما تشاء.<br/><br/>يظهر السطر الأول المخزون الحالي. في السطر الثاني، يمكنك اختيار توزيع آخر. السطر الثالث يبين الفرق بين المخزون القديم والجديد.</p><input
    ...
    <table id="npc" cellpadding="1" cellspacing="1">
        ...
        <tr>
            ...
            <td class="deco"></td>
            <td class="sum">يتبقى:&nbsp;<span id="org4">'.($vilres["wood"]+$vilres["clay"]+$vilres["iron"]+$vilres["crop"]).'</span></td>
        </tr>
        </thead>
        <tbody>
        <tr>
            ...
            <td class="deco"></td>
            <td class="sum">المجموع:&nbsp;<span id="newsum">'.($_POST['defaultValues']['r1']+$_POST['defaultValues']['r2']+$_POST['defaultValues']['r3']+$_POST['defaultValues']['r4']).'</span></td>
        </tr>
        <tr>
            ...
            <td class="deco"></td>
            <td class="sum">يتبقي:&nbsp;<span id="remain">'.($vilres["wood"]+$vilres["clay"]+$vilres["iron"]+$vilres["crop"]).'</span></td>
        </tr>
        </tbody>
    </table>
    <p id="submitButton" class="disableButtonHandler">
        <button type="submit" value="Redeem" id="npc_market_button" class="gold " title="توزيع الآن" coins="3">
            <div class="button-container addHoverClick">
                <div class="button-background">
                    <div class="buttonStart">
                        <div class="buttonEnd">
                ...
                <div class="button-content">توزيع الآن<img src="img/x.gif" class="goldIcon" alt=""/><span class="goldValue">3</span>
                </div>
            </div>
        </button>
    ...
    <p id="submitText">
        <button type="button" id="button54872383d13d2" class="gold "
            ...
            <div class="button-container addHoverClick">
                ...
                <div class="button-content">توزيع الموارد المتبقية</div>
            </div>
        </button>
...
/*
                        ...
                        switch($_POST['questTutorialId']){
                            case "AchievementQuestReward_01":
                                ...
                                if($rData[1]==0){
                                    ...
                                    if($rData[0]==0){
                                        $database->query("UPDATE `achiev` SET `reward1`='".$reward.",0'  WHERE `uid`='".$session->uid."'");
                                    }
                                    
                                }
                                $prize=constant("questday".($rData[0]+4));
                                $prizes="<li>".questday5."</li><li>".questday6."</li><li>".questday7."</li>";
                                break;

                            case "AchievementQuestReward_02":
                                $points=50;
                                $reward=$achievs['reward2'];
                                if($achievs['reward2']==0){
                                    $reward=rand(1,5);
                                    switch($reward) {
                                        case 1:  $plus="plus"; break;
                                        case 2:  $plus="b1";   break;
                                    ...
                                    $q = "UPDATE users SET `".$plus."`='".$time."' where  `id` =  '".$session->uid."'";
                                    $database->query($q);
                                    $database->query("UPDATE `achiev` SET `reward2`='".$reward."'  WHERE `uid`='".$session->uid."'");
                                }


                                $prize=constant("questday".($reward+8));
                                $prizes=" <li>".questday9."</li><li>".questday10."</li><li>".questday11."</li><li>".questday12."</li><li>".questday13."</li>";
                                break;

                            case "AchievementQuestReward_03":
                                $points=75;
                                $reward=$achievs['reward3'];
                                if($achievs['reward3']==0){
                                    $reward=rand(1,3);
                                    switch($reward){
                                        case 1:
                                            $peoples=$database->query("SELECT wref FROM `vdata` WHERE `owner`='".$session->uid."' LIMIT 1");
                                    ...
                                    $database->query("UPDATE `achiev` SET `reward3`='".$reward."'  WHERE `uid`='".$session->uid."'");
                                }


                                $prize=constant("questday".($reward+14));
                                $prizes="<li>".questday15."</li><li>".questday16."</li><li>".questday17."</li>";
                                break;
                            case "AchievementQuestReward_04":
                                $points=100;
                                $reward=$achievs['reward4'];
                                if($achievs['reward4']==0){
                                    $reward=rand(1,3);
                ...
                $database->query("UPDATE `achiev` SET `reward4`='".$reward."'  WHERE `uid`='".$session->uid."'");


                                }
                                $prize=constant("questday".($reward+18));
                                $prizes="<li>".questday19."</li><li>".questday20." </li><li>".questday21." </li>";
                                break;

                        }
                $html = json_encode('<div class="questWrapper achievements">
                <h2 class="questTitle">تهانينا! لقد قمت بتجميع نقاط كافية للحصول على مكافأة</h2>
                <hr class="achievementLine">
                <div class="questImage"><img id="questLogo" src="img/x.gif" class="enumerableElementsImage AchievementQuestReward_01" style="" alt="25 نقاط جمعت">
                </div>
                <div class="questDescription">
                    <div id="questDescription" class="enumerableElementsDiscription " style="" title="">كونك قمت بتجميع 25 نقاط/نقطة اليوم، تحصل على ما يلي 25			<br>
                        يتم تحديد المكافأة اليومية بشكل عشوائي من هذه الخيارات
                        :
                        <br>
                        <br>
                        <ul>
                    ...
                    <h3 class="questRewardTitle">'.$prize.'</h3>
                    '.$prize.'	</div>
                <div class="clear"></div>
                <hr class="achievementLine">
                <div class="questButtons">
                    <button type="submit" value="نظرة عامة" id="buttonGEhpYFPK2brYO" class="green questButtonOverviewAchievements" questbuttonoverviewachievements="1">
                        <div class="button-container addHoverClick">
                            <div class="button-background">
                                <div class="buttonStart">
                                    <div class="buttonEnd">
                            ...
                            <div class="button-content">نظرة عامة</div>
                        </div>
                    </button>
                    <script type="text/javascript">
                        window.addEvent(\'domready\', function () {
                    ...
                    <button type="submit" value="جمع المكافآت" id="buttondUrnNtIEZBy9H" class="green questButtonGainReward" questid="AchievementQuestReward_01" questbuttongainreward="1">
                        <div class="button-container addHoverClick">
                            <div class="button-background">
                                <div class="buttonStart">
                                    <div class="buttonEnd">
                            ...
                            <div class="button-content">جمع المكافآت</div>
                        </div>
                    </button>
                    <script type="text/javascript">
                        window.addEvent(\'domready\', function () {
                                        ...
                                        window.fireEvent(
                                            \'buttonClicked\', [
                                                this, {
                                                    ...
                                                    "confirm": "",
                                                    "onclick": "",
                                                    "questId": "AchievementQuestReward_01",
                ...
                ".questday37."<br />
                <ul>".$prizes."</ul></div>
                        <h3 class=\"questRewardTitle\">".questday38." ".$points." ".questday39."</h4>
                        ".$prize."    </div>
                ...
                <div class=\"clear\"></div>
                            ...
                            echo '{"response": {"error":false,"errorMsg":null,"data":{"html":'.$html.'}}}';
...
*/
                        ...
                        case "Tutorial_01":
                        //print_r($_POST); die();
                        //preg_match_all('!\d+!', $_POST['questTutorialId'], $idQuest);
                        //echo (int)preg_replace('/[^0-9]/', '', $_POST['questTutorialId']);
                        //die();
...
</ul>":"جائزتك لليوم:<br /> ".constant("questday".($rData2[0]+8));
...
</ul>":"جائزتك لليوم:<br /> ".constant("questday".($rData3[0]+13));
...
<div class=\"achievement\" ><h1 class=\"questList\">".questday22."</h1>
    ...
    <tr class=\"zebra\">
            ...
            قم بعمل احتفال كبير أو احتفال صغير
            ".($acData['a10'] < 15 ? '</a>' : '')."
            </td>
...
for($i=19;$i<=40;$i++){
	if($builds['f'.$i.'t']==12){
        ...
        break;
                ...
                case 'paymentProviders':
                    ...
                    <div class="methodItem" title="الدفع عبر البنك">
                        <input type="hidden" class="providerId" value="11"\/><img src="img/provider/Logo-ARB.png" alt="الدفع البنكي">
                    </div>
                    </div>';
                    echo '{"response": {"error":false,"errorMsg":null,"data":{"html":'.json_encode($html).'}}}';
                break;
            ...
            ';
                ...
                <div class="contentBorder-br"></div>
                <div class="contentBorder-bc"></div>
                <div class="contentBorder-contents cf"><img src="img/x.gif" class="mentor vid_2" alt="المستشار">
                </div>
            </div>
                ...
                <div class="contentBorder-contents cf">
                        <a href="#" onclick="jQuery(\'.paymentWizardMenu\').addClass(\'hide\');jQuery(\'.buyGoldInfoStep\').removeClass(\'active\');jQuery(\'.buyGoldInfoStep#1\').addClass(\'active\');jQuery(\'.paymentWizardMenu#generalOptions\').removeClass(\'hide\');">
                            <div class="buyGoldInfoStep active" id="1">
                                ...
                                <div class="buyGoldInfoStepLabel">عام                        :
                                </div>
                                <div class="buyGoldInfoStepContent">خيارات عامة</div>
                            </div>
                        </a>
                        <a href="#" onclick="jQuery(\'.paymentWizardMenu\').addClass(\'hide\');jQuery(\'.buyGoldInfoStep\').removeClass(\'active\');jQuery(\'.buyGoldInfoStep#2\').addClass(\'active\');jQuery(\'.paymentWizardMenu#buyResources\').removeClass(\'hide\');">
                            <div class="buyGoldInfoStep " id="2">
                                ...
                                <div class="buyGoldInfoStepLabel">القوات                        :
                                </div>
                                <div class="buyGoldInfoStepContent">شراء موارد</div>
                            </div>
                        </a>
                        <a href="#" onclick="jQuery(\'.paymentWizardMenu\').addClass(\'hide\');jQuery(\'.buyGoldInfoStep\').removeClass(\'active\');jQuery(\'.buyGoldInfoStep#3\').addClass(\'active\');jQuery(\'.paymentWizardMenu#moreProtection\').removeClass(\'hide\');">
                            <div class="buyGoldInfoStep " id="3">
                                ...
                                <div class="buyGoldInfoStepLabel">الحماية:
                                </div>
                                <div class="buyGoldInfoStepContent">حزم الحماية</div>
                            </div>
                        </a>
                        <!--
                        <a href="#" onclick="jQuery(\'.paymentWizardMenu\').addClass(\'hide\');jQuery(\'.buyGoldInfoStep\').removeClass(\'active\');jQuery(\'.buyGoldInfoStep#4\').addClass(\'active\');jQuery(\'.paymentWizardMenu#power\').removeClass(\'hide\');">
                            <div class="buyGoldInfoStep " id="4">
                                ...
                                <div class="buyGoldInfoStepLabel">القوة!</div>
                                <div class="buyGoldInfoStepContent">مكافأة الهجوم/الدفاع</div>
                            </div>
                        </a>-->
                            <script type="text/javascript">
                        function productPurchased(data) {
                            ...
                            if (data.type === \'animal\') {
                                dialog.setContent(\'الوحوش التي تم شرائها\');
                            } else if (data.type === \'troops\') {
                                dialog.setContent(\'الجيوش التي تم شرائها\');
                            } else {
                                dialog.setContent(\'الموارد التي تم شرائها\');
                            }
                            dialog.show();
            ...
            <div class="contentBorder contentArea">
                ...
                <div class="contentBorder-contents cf">
                    <div class="paymentPopupDialogWrapper">
                        <div class="paymentWizardMenu " id="generalOptions">
                            <div class="feature featureBooking "><input type="hidden" class="premiumFeatureName hide" name="featureName" value="upgradeAllResourcesTo20"><div class="featureContent"><h3 class="featureTitle">ترقية الموارد الحقول إلى المستوى 20</h3><div class="featureRemainingTime featureSubtitle subtitle"><span class="">ترقية الموارد الحقول إلى المستوى 20</span></div><div class="featureButton">
                    ...
                    <div class="button-content">شراء<img src="img/x.gif" class="goldIcon" alt=""><span class="goldValue">'.(MAX_LEVEL_COST * 18).'</span></div></div>
                    </button>
                    ...
                    <div class="featureDuration featureRenewal featureButtonSubtitle subtitle">توصيل: <span class="bold">فوراً</span>    </div></div></div>

                    '.($config['storageUpgrade'] ? '<div class="feature featureBooking "><input type="hidden" class="premiumFeatureName hide" name="featureName" value="increaseStorage"><div class="featureContent"><h3 class="featureTitle">زيادة تخزين</h3><div class="featureRemainingTime featureSubtitle subtitle"><span class="">زيادة سعة التخزين الحالية بمستوى تخزين 20.</span></div><div class="featureButton"><button type="button" class="gold prosButton increaseStorage" coins="'.$config['storageUpgrade'].'" id="buttonV2" onclick=""><div class="button-container addHoverClick">
                    <div class="button-background">
                        <div class="buttonStart">
                            <div class="buttonEnd">
                                <div class="buttonMiddle"></div>
                    ...
                    <div class="button-content">شراء<img src="img/x.gif" class="goldIcon" alt=""><span class="goldValue">'.$config['storageUpgrade'].'</span></div></div></button>
                ...
                </script></div><div class="featureDuration featureRenewal featureButtonSubtitle subtitle">توصيل: <span class="bold">فوراً</span>    </div></div></div>' : '').'
                    
                
                <div class="feature featureBooking "><input type="hidden" class="premiumFeatureName hide" name="featureName" value="finishTraining"><div class="featureContent"><h3 class="featureTitle">الانتهاء من جميع قوات التدريب فوراً</h3><div class="featureRemainingTime featureSubtitle subtitle"><span class="">كل ما تبذلونه من القوات على قائمة الانتظار في القرية الحالية سوف تدرب على الفور.</span></div><div class="featureButton"><button type="button" class="gold prosButton finishTraining" coins="1" id="buttonH3" onclick=""><div class="button-container addHoverClick">
                    <div class="button-background">
                        <div class="buttonStart">
                            <div class="buttonEnd">
                                <div class="buttonMiddle"></div>
                    ...
                    <div class="button-content">شراء<img src="img/x.gif" class="goldIcon" alt=""><span class="goldValue">'.FINISH_ALL_COST.'</span></div></div></button>
            ...
            '.($config['25pFaster'] ? '<div class="feature featureBooking "><input type="hidden" class="premiumFeatureName hide" name="featureName" value="fasterTraining"><div class="featureContent"><h3 class="featureTitle">+25% تدريب أسرع</h3><div class="featureRemainingTime featureSubtitle subtitle"><span class="">يمكنك تدريب أسرع في كل من قريتك في هذه الفترة.</span></div><div class="featureButton"><button type="button" class="gold prosButton fasterTraining" coins="'.$config['25pFaster'].'" id="buttonn4" onclick=""><div class="button-container addHoverClick">
            <div class="button-background">
                <div class="buttonStart">
                    <div class="buttonEnd">
                        <div class="buttonMiddle"></div>
            ...
            <div class="button-content">شراء<img src="img/x.gif" class="goldIcon" alt=""><span class="goldValue">'.$config['25pFaster'].'</span></div></div></button>
        ...
        '.($p->pAData('fasterTraining') > time() ? '<div class="featureDuration featureRenewal featureButtonSubtitle subtitle"><span class="renewalActive"> تنتهي في <span class="timer" counting="down" value="'.($p->pAData('fasterTraining')-time()).'">'.$generator->getTimeFormat($p->pAData('fasterTraining')-time()).'</span>.</span></div>' : 
        '<div class="featureDuration featureRenewal featureButtonSubtitle subtitle">مدة المكافأة: <span class="bold">12</span>  ساعه  </div>').'</div></div>' : '').'
        
        '.($config['allSmithy'] ? '<div class="feature featureBooking "><input type="hidden" class="premiumFeatureName hide" name="featureName" value="smithyUpgradeAllToMax"><div class="featureContent"><h3 class="featureTitle">ترقية جميع القوات إلى مستوى الحد الأقصى في افران صهر الحديد</h3><div class="featureRemainingTime featureSubtitle subtitle"><span class="">وسيتم ترقية جميع القوات إلى مستوى الحد الأقصى في أفران صهر الحديد.</span></div><div class="featureButton"><button type="button" class="gold prosButton smithyUpgradeAllToMax" coins="'.$config['allSmithy'].'" id="buttonI5" onclick=""><div class="button-container addHoverClick">
        <div class="button-background">
            <div class="buttonStart">
                <div class="buttonEnd">
                    <div class="buttonMiddle"></div>
        ...
        <div class="button-content">شراء<img src="img/x.gif" class="goldIcon" alt=""><span class="goldValue">'.$config['allSmithy'].'</span></div></div></button>
    ...
    '.($config['searchAll'] ? '<div class="feature featureBooking "><input type="hidden" class="premiumFeatureName hide" name="featureName" value="academyResearchAll"><div class="featureContent"><h3 class="featureTitle">البحث عن جميع الوحدات</h3><div class="featureRemainingTime featureSubtitle subtitle"><span class="">بحث جميع الوحدات في القرية. لا حاجة للبحث في الأكاديمية بعد الآن.</span></div><div class="featureButton">
    <button type="button" '.($p->academyAll() ? '' : 'title="تم البحث عن جميع الوحدات بالقرية."').' class="gold prosButton academyResearchAll '.($p->academyAll() ? '' : 'disabled').'" coins="'.$config['searchAll'].'" id="buttonL6" onclick=""><div class="button-container addHoverClick">
        <div class="button-background">
            <div class="buttonStart">
                <div class="buttonEnd">
                    <div class="buttonMiddle"></div>
        ...
        <div class="button-content">شراء<img src="img/x.gif" class="goldIcon" alt=""><span class="goldValue">'.$config['searchAll'].'</span></div></div></button>
    ...
    window.addEvent(\'domready\', function () {
        if ($(\'buttonL6\')) {
            $(\'buttonL6\').addEvent(\'click\', function () {
                window.fireEvent(\'buttonClicked\', [this, {"type":"button","value":"\u0634\u0631\u0627\u0621","confirm":"","onclick":"","wayOfPayment":{"featureKey":"academyResearchAll","context":"paymentWizard"},"title":"\u0634\u0631\u0627\u0621","coins":'.$config['searchAll'].',"id":"buttonL6"}]);
    ...
    </div><div class="featureDuration featureRenewal featureButtonSubtitle subtitle">توصيل: <span class="bold">فوراً</span>    </div></div></div>' : '').' </div>
                    ...
                    <div class="button-content">شراء<img src="img/x.gif" class="goldIcon" alt=""><span class="goldValue">'.$config['resources01'].'</span></div></div></button>
                    ...
                    <div class="button-content">شراء<img src="img/x.gif" class="goldIcon" alt=""><span class="goldValue">'.$config['resources02'].'</span></div></div></button>
                ...
                <div class="button-content">شراء<img src="img/x.gif" class="goldIcon" alt=""><span class="goldValue">'.$config['resources03'].'</span></div></div></button>
            ...
            '.($config['protect01'] ? '' : '').'
                ...
                <div class="feature featureBooking "><input type="hidden" class="premiumFeatureName hide" name="featureName" value="defBonus"><div class="featureContent"><h3 class="featureTitle">+15% مكافأة الدفاع</h3><div class="featureRemainingTime featureSubtitle subtitle"><span class="">زيادة القدرة الدفاعية للقوات اثناء مرافقة البطل</span></div><div class="featureButton"><button type="button" class="gold prosButton defBonus" coins="100" id="buttonR11" onclick=""><div class="button-container addHoverClick">
                    <div class="button-background">
                        <div class="buttonStart">
                            <div class="buttonEnd">
                                <div class="buttonMiddle"></div>
                    ...
                    <div class="button-content">شراء<img src="img/x.gif" class="goldIcon" alt=""><span class="goldValue">100</span></div></div></button>
                ...
                </script></div><div class="featureDuration featureRenewal featureButtonSubtitle subtitle">مدة المكافأة: <span class="bold">10</span>  ساعه  </div></div></div>                                    </div>
                <div class="paymentWizardMenu hide" id="moreProtection">';
                    if($session->protect){
                        $html .= '<h5 class="round">لديك '.$generator->getTimeFormat(($session->protection-time())).' ساعة(s) اخرى من الحماية</h5>';
                    }else{
                        $html .= '<h5 class="round">إنتهت حماية المبتدئين لديك.</h5>';
                    }
                    
                    $html .= '<br>
                    '.($config['protect01'] ? '<div class="feature featureBooking "><input type="hidden" class="premiumFeatureName hide" name="featureName" value="moreProtection0"><div class="featureContent"><h3 class="featureTitle">الحماية</h3><div class="featureRemainingTime featureSubtitle subtitle"><span class="">مع الحماية لا يمكن لأحد أن يهاجمك وأنت بالخارج.</span></div><div class="featureButton"><button type="button" class="gold prosButton moreProtection0" coins="'.$config['protect01'].'" id="buttonb12" onclick=""><div class="button-container addHoverClick">
                    <div class="button-background">
                        <div class="buttonStart">
                            <div class="buttonEnd">
                                <div class="buttonMiddle"></div>
                    ...
                    <div class="button-content">شراء<img src="img/x.gif" class="goldIcon" alt=""><span class="goldValue">'.$config['protect01'].'</span></div></div></button>
                ...
                </script></div><div class="featureDuration featureRenewal featureButtonSubtitle subtitle">مدة المكافأة: <span class="bold">1</span>  ساعه  </div></div></div>' : '').'
                    
                '.($config['protect02'] ? '<div class="feature featureBooking "><input type="hidden" class="premiumFeatureName hide" name="featureName" value="moreProtection1"><div class="featureContent"><h3 class="featureTitle">الحماية</h3><div class="featureRemainingTime featureSubtitle subtitle"><span class="">مع الحماية لا يمكن لأحد أن يهاجمك وأنت بالخارج.</span></div><div class="featureButton"><button type="button" class="gold prosButton moreProtection1" coins="'.$config['protect02'].'" id="buttonT13" onclick=""><div class="button-container addHoverClick">
                <div class="button-background">
                    <div class="buttonStart">
                        <div class="buttonEnd">
                            <div class="buttonMiddle"></div>
                ...
                <div class="button-content">شراء<img src="img/x.gif" class="goldIcon" alt=""><span class="goldValue">'.$config['protect02'].'</span></div></div></button>
            ...
            </script></div><div class="featureDuration featureRenewal featureButtonSubtitle subtitle">مدة المكافأة: <span class="bold">3</span>  ساعه  </div></div></div>' : '').'
                
            '.($config['protect03'] ? '<div class="feature featureBooking "><input type="hidden" class="premiumFeatureName hide" name="featureName" value="moreProtection2"><div class="featureContent"><h3 class="featureTitle">الحماية</h3><div class="featureRemainingTime featureSubtitle subtitle"><span class="">مع الحماية لا يمكن لأحد أن يهاجمك وأنت بالخارج.</span></div><div class="featureButton"><button type="button" class="gold prosButton moreProtection2" coins="'.$config['protect03'].'" id="buttonq14" onclick=""><div class="button-container addHoverClick">
            <div class="button-background">
                <div class="buttonStart">
                    <div class="buttonEnd">
                        <div class="buttonMiddle"></div>
            ...
            <div class="button-content">شراء<img src="img/x.gif" class="goldIcon" alt=""><span class="goldValue">'.$config['protect03'].'</span></div></div></button>
        ...
        window.addEvent(\'domready\', function () {
            if ($(\'buttonq14\')) {
        ...
        </script></div><div class="featureDuration featureRenewal featureButtonSubtitle subtitle">مدة المكافأة: <span class="bold">6</span>  ساعه  </div></div></div>' : '').'
                ...
                $html .= '<div class="header">
                            <ul>
                                ...
                                <a href="#" class="tabButton paymentFeatures" onclick="return false;">
                                    ...
                                    <div class="text"><img src="img/starGold.png" style="vertical-align: middle;width: 22px;">&nbsp;ميزات ترافيان بلاس							</div>
                                    <!--  <img src="img/x.gif" class="tabBtnImg"> -->
                                </a>
                ...
                case 'bb':
                    ...
                    echo '{"response":{"error":false,"errorMsg":null,"data":{"text":"'.stripslashes(nl2br($bbcoded)).'"}}}';
                break;
                ...
                case 'ignoreList':       
                    ...
                    if($_POST['method'] == 'render_ignore_list'){
                        ...
                        $html .= '<div class="clear"></div><div><table><tfoot><tr><td colspan="6">'.$i.'/20</td></tr><tr><td colspan="6">لتجاهل الرسائل من لاعب معين، انتقل إلى ملفه الشخصي وانقر على "تجاهل"!</td></tr></tfoot></table></div>';
                        ...
                        if($database->getUserInfo($_POST['params']['targetPlayer'])['id']){
                            ...
                            //echo '{"response":{"error":false,"errorMsg":null,"data":{"result":"<a class=\"message messageStatus messageStatusUnread\" href=\"nachrichten.php?t=1&amp;id='.$_POST['params']['targetPlayer'].'\">\u0627\u0643\u062a\u0628 \u0631\u0633\u0627\u0644\u0629<\/a><br \/><br \/><a href=\"\" id=\"ignore-player-button\" data-player-ignored=\"false\" data-uid=\"'.$_POST['params']['targetPlayer'].'\" title=\"\u062a\u062c\u0627\u0647\u0644 \u0627\u0644\u0644\u0627\u0639\u0628\">\u062a\u062c\u0627\u0647\u0644 \u0627\u0644\u0644\u0627\u0639\u0628.<\/a>"}}}';
                        }
                        ...
                        if($_POST['renderIgnoreList'] == 'true'){
                            ...
                            $html .= '<div class="clear"></div><div><table><tfoot><tr><td colspan="6">0/20</td></tr><tr><td colspan="6">لتجاهل الرسائل من لاعب معين، انتقل إلى ملفه الشخصي وانقر على "تجاهل"!</td></tr></tfoot></table></div>';
                            echo '{"response":{"error":false,"errorMsg":null,"data":{"result":'.json_encode($html).'}}}';    
                        }else{
                            echo '{"response":{"error":false,"errorMsg":null,"data":{"result":"<a class=\"message messageStatus messageStatusUnread\" href=\"nachrichten.php?t=1&amp;id='.$_POST['params']['targetPlayer'].'\">\u0627\u0643\u062a\u0628 \u0631\u0633\u0627\u0644\u0629<\/a><br \/><br \/><a href=\"\" id=\"ignore-player-button\" data-player-ignored=\"false\" data-uid=\"'.$_POST['params']['targetPlayer'].'\" title=\"\u062a\u062c\u0627\u0647\u0644 \u0627\u0644\u0644\u0627\u0639\u0628\">\u062a\u062c\u0627\u0647\u0644 \u0627\u0644\u0644\u0627\u0639\u0628.<\/a>"}}}';
                        ...
                        if($_POST['method'] == 'render_player_message_ignore_buttons'){
                            if($database->queryFetch('SELECT COUNT(*) AS num FROM `ignore` WHERE `uid` = '.$session->uid.' AND `uignored` = '.$_POST['params']['targetPlayer'].'')['num'] == 0){
                                ...
                                echo '{"response":{"error":false,"errorMsg":null,"data":{"result":"<span class=\"notice\">\u0633\u064a\u062a\u0645 \u062a\u062c\u0627\u0647\u0644 \u0644\u0627\u0639\u0628.<\/span><br \/><a href=\"\" id=\"ignore-player-button\" data-player-ignored=\"true\" data-uid=\"'.$_POST['params']['targetPlayer'].'\" title=\"\u0642\u0628\u0648\u0644 \u0627\u0644\u0631\u0633\u0627\u0626\u0644 \u0645\u0646 \u0647\u0630\u0627 \u0627\u0644\u0644\u0627\u0639\u0628\">\u062a\u0648\u0642\u0641 \u0639\u0646 \u062a\u062c\u0627\u0647\u0644 \u0647\u0630\u0627 \u0627\u0644\u0644\u0627\u0639\u0628.<\/a>"}}}';
                            }
                            ...
                            if($_POST['method'] == 'ignore_target_player'){
                                ...
                                if($database->queryFetch('SELECT COUNT(*) AS num FROM `ignore` WHERE `uid` = '.$session->uid.'')['num'] == 20){
                                    echo '{"response":{"error":true,"errorMsg":"وصلت للحد الأقصي من الاعبين المتاجهلين.","data":[]}}';
                                }else{
                ...
                case 'checkRecipient':
                    ...
                    if($usData['id'] || $uName = '[ally]; [ally]'){
                        ...
                        if($cIgnore != 0 && $session->access !=9 && $uName != '[ally]; [ally]'){
                            echo '{"response":{"error":true,"errorMsg":"لا يمكنك إرسال رسائل لهذا الاعب، الاعب أضافك لقائمة التجاهل.","data":[]}}';
                        }else{
                            ...
                            if($totalpop < 100 && $session->access != 9){
                                echo '{"response":{"error":true,"errorMsg":"لا يمكنك إرسال رسائل وسكانك أقل من 100.","data":[]}}';
                            }else{
                        ...
                        echo '{"response":{"error":true,"errorMsg":"\u0647\u0630\u0627 \u0627\u0644\u0627\u0633\u0645  \u063a\u064a\u0631 \u0645\u0648\u062c\u0648\u062f","data":[]}}';
                    }
...
(
    ...
    [ajaxToken] => de3768730d5610742b5245daa67b12cd
                    ...
                    switch($_POST['action']){
                        case 'actionAddListForm':
                            ...
                            <form action="?" method="post">
                                ...
                                <br>
                                <button type="button" value="انشاء" id="buttonfF9BNzkGyFEj7" class="green " onclick="Travian.Game.RaidList.createNewList()">
                                    <div class="button-container addHoverClick">
                                        <div class="button-background">
                                            <div class="buttonStart">
                                                <div class="buttonEnd">
                                        ...
                                        <div class="button-content">انشاء</div>
                                    </div>
                                </button>
                                <script type="text/javascript">
                                    window.addEvent(\'domready\', function () {
                        ...
                        case 'actionAddList':
                            if(empty($_POST['listName']) || empty($_POST['did'])){
                                echo $ajax->error('أضف إسم للقائمة'); exit();
                            }else{
                                if(strlen($_POST['listName']) > 12){
                                    echo $ajax->error('إسم القائمة طويل  جدا'); exit();
                                }else{
                ...
                case 'demolishNowPopup':
                    //print_r($_POST); die();
                    $html = '<div id="demolishNowDialog"><p>هل تريد بالتأكيد هدم هذا المبنى تماما؟ سيتم إزالة هذا المبنى من قريتك.</p>
                    <div class="buttonWrapper"><button type="submit" class="gold " coins="'.DEMOLISH_FULL.'" id="buttonalA6HUXzDkKj3"><div class="button-container addHoverClick">
                        ...
                        <div class="button-content">إنهي<img src="img/x.gif" class="goldIcon" alt=""><span class="goldValue">'.DEMOLISH_FULL.'</span></div></div></button>
